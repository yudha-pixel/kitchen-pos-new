...
export default function CRMPage() {
  ...
  useEffect(() => {
    const handleMemberUpdated = () => {
      console.log('📡 Received memberUpdated event, refreshing CRM data...');
      loadMembers();
    };

    if (typeof window !== 'undefined') {
      window.addEventListener('memberUpdated', handleMemberUpdated);
      console.log('🔍 [CRM Page] Listening for memberUpdated events');
    }

    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener('memberUpdated', handleMemberUpdated);
        console.log('🔍 [CRM Page] Stopped listening for memberUpdated events');
      }
    };
  }, []);
